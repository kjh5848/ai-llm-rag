...
## 7. Mermaid 다이어그램 작성 규칙 (Syntax Guide)

마크다운 내에 삽입되는 Mermaid 다이어그램 작성 시, 렌더링 오류를 방지하기 위해 다음 문법을 엄격히 준수합니다.

### 7.1 연결선(Edge) 텍스트에 괄호 문법 적용
노드와 노드를 잇는 연결선 텍스트(Label)에 괄호 `()` 등 특수문자가 포함된 경우, 파싱 에러(Expected SQE, got PS 등)가 빈번히 발생합니다.
따라서 **텍스트가 포함된 연결선을 작성할 때는 반드시 `A -- "텍스트" --> B` 형식을 사용**하며, 텍스트 전체를 쌍따옴표 `""`로 감싸야 합니다.

- **잘못된 예시** (파싱 에러 발생 위험):
  ```mermaid
  A -->|5번 스크립트(Chunking)| B
  A -->|"5번 스크립트(Chunking)"| B
  ```
- **올바른 예시** (안전한 문법):
  ```mermaid
  A -- "5번 스크립트(Chunking & Embedding)" --> B
  A -. "점선 텍스트(예시)" .-> B
  A == "굵은선 텍스트(예시)" ==> B
  ```
